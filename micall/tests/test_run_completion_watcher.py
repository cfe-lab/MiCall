...
class TestMonitorRunCompletion:
    ...
    def test_monitor_runs_creates_needsprocessing_for_stable_runs(self, tmp_path):
        ...
        runs_dir = tmp_path
        run1 = runs_dir / "230224_M04401_0030_000000000-STABLE1"
        basecalls_dir1 = run1 / "Data/Intensities/BaseCalls"
        basecalls_dir1.mkdir(parents=True)
        for i in range(3):
            (basecalls_dir1 / f"file{i}.fastq.gz").touch()

        run2 = runs_dir / "230224_M04401_0031_000000000-STABLE2"
        alignment_dir = run2 / "Alignment_1/20251101/Fastq"
        alignment_dir.mkdir(parents=True)
        for i in range(2):
        ...
        assert not (run1 / "needsprocessing").exists(), (
            "Run 1 should not have needsprocessing marker"
        )
        assert not (run2 / "needsprocessing").exists(), (
            "Run 2 should not have needsprocessing marker"
        )
        ...
        with patch(
            ...
            monitoring = []
            check_run_completions(runs_dir, monitoring, dry_run=False)
            check_run_completions(runs_dir, monitoring, dry_run=False)
            check_run_completions(runs_dir, monitoring, dry_run=False)
