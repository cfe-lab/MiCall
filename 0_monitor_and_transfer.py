...
## Settings
pipeline_version = "4.4c-HCV-csf-only"
...
mpi_run = "/opt/scyld/openmpi/1.7.2/gnu/bin/mpirun"
...
while 1:
	...
	runs = glob(macdatafile_mount + 'MiSeq/runs/*A3T6H/needsprocessing')
	runs += glob(macdatafile_mount + 'MiSeq/runs/*A3T67/needsprocessing')
	...
	with open(MPI_log_path, "wb") as MPI_wrapper_log:
		...
		command = [mpi_run, '-tag-output', '-machinefile', 'mfile', 'python', '-u', '1_MPI_wrapper.py', home+run_name]
